...
import { googleConstants } from "../../global/app.settings";
...
import { GoogleOidcConstants } from "../auth.interfaces";

export const buildOpenIdClient = async (googleConstants: GoogleOidcConstants) => {
...
@Injectable()
export class GoogleOidcStrategy {

    client: Client = null; //null will be replaced the first time that buildOpenIdClient is called.

    /*manually create service of openid-client
    See https://developer.aliyun.com/mirror/npm/package/openid-client-request#example,
    https://openbase.io/js/openid-client/documentation#authorization-code-flow and former
    ...
    */
    ...
    async getOpenIdClient(): Promise<Client> {
        try {
            ...
            if (!client) {//if client is null, instantiate the client
                //console.log('need to build openId client');
                client = await buildOpenIdClient(googleConstants);
                //save off the client connection built so that it is not repeated in subsequent requests.
                this.client = client;
            }
            return client;
        }catch(error){
            console.log(`Could not create Google login client: ${error}`)
            throw new InternalServerErrorException(`Could not create Google login client: ${error}`);
        } 
    }
    ...
    async getAuthorizationUrl(req: Request) {
        ...
        return (await this.getOpenIdClient()).authorizationUrl({
    ...
    async processCallBack(req: any) {
        const params = (await this.getOpenIdClient()).callbackParams(req);
        //console.log(req.session['code_verifier']);
        const tokenSet = await (await this.getOpenIdClient()).callback(googleConstants.GOOGLE_OAUTH2_REDIRECT_URI, params, { code_verifier: req.session.get('code_verifier') }) // => Promise
